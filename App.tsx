...
import { Lock, Smartphone, Bell, Mail, Calculator as CalcIcon, Settings, Home, LayoutDashboard, Menu, X, Layers, CreditCard } from 'lucide-react';
import { AnimatePresence, motion } from 'framer-motion';

const AppContent: React.FC = () => {
  const { currentMood, setMood, currentTheme, activeScreen, navigateTo, isAppDrawerOpen } = useMood();
    ...
    <div className="flex h-screen w-full bg-[#0a0a0a] text-gray-100 overflow-hidden font-sans selection:bg-blue-500/30">
      ...
      <div className={`${isSidebarOpen ? 'w-72 opacity-100 translate-x-0' : 'w-0 opacity-0 -translate-x-10 overflow-hidden'} transition-all duration-300 flex flex-col border-r border-white/5 bg-[#0f0f11] p-0 z-40 shadow-2xl`}>
        ...
        <div className="flex-1 overflow-y-auto p-6 scrollbar-hide">
          ...
          {/* AI Camera Section */}
          <div className="mb-8">
            <h2 className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-4">Gemini Vision</h2>
            <CameraCapture />
          </div>
